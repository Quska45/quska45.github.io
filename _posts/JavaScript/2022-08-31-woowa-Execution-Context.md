...
여기서 goTo2F의 outer는 Global의 실행컨텍스를 참조하게 됩니다.
추상화된 코드의 관점에서 보면 1층과 2층의 사다리 역할을 outer가 한다고 볼 수 있습니다.
<br/>
<br/>
...
코드가 조금 복잡했지만 어려울 것은 없습니다.
goTo3F의 실행컨텍스트가 생성됐고 outer는 goTo2F의 실행컨텍스트가 참조됩니다.
추상화된 코드의 관점에서는 3층이 생성 됐고 2층과 3층의 사다리도 함께 생성된 것이죠.
<br/>
<br/>
그럼 chair를 출력할 때는 어떤 일이 일어날까요?
자바스크립트는 먼저 goTo3F에서 chair를 찾습니다.
3층에는 chair가 존재하지 않기 때문에 outer라는 사다리를 통해 연결된 2층을 찾아 보게 됩니다.
2층에도 chair가 존재하지 않는다면 1층까지 가겠죠.
결과적으로 chair는 존재 하지 않기 때문에 예외가 발생하게 됩니다.
<br/>
<br/>
그럼 3층의 lamp는 어떻게 되는 걸까요?
먼저 outer를 통해 2층으로 갑니다.
다행히 바로 램프를 찾고 on을 출력합니다.
2층에서 lamp를 찾았기 때문에 굳이 1층까지 가는 수고를 할필요가 없게 됩니다.
자바스크립트의 이런 현상을 `변수 섀도잉`이라고 부릅니다.
동일한 식별자로 인해 상위 스코프에서 선언된 식별자의 값이 가려지는 현상 입니다.

### 3-3. Scope Chain
새로운 용어지만 방금까지 봤던 개념입니다.
식별자를 결정할 때 활용하는 스코프들의 연결리스트를 스코프 체인이라고 합니다.
앞서 봤던 코드를 통해 생성된 lexical 컨택스트들이 outer를 통해 만들어낸 관계가 스코프 체인 입니다.

## 4. Execution Context(실행 컨텍스트) 정리
실행 컨택스트는 코드를 실행하는데 필요한 환경을 제공하는 객체입니다.
여기서 말하는 환경은 코드 실행에 영향을 주는 조건이나 상태 입니다.
이런 환경을 모아놓은 객체가 실행 컨텍스트라고 할 수 있습니다.
<br/>
<br/>
자바스크립트가 지금의 성능을 낼 수 있는 이유 중에 하나는 실행 컨텍스트 덕분입니다.
만약 실행컨텍스트가 없었다면 여러개의 분리된 기능을 하는 객체들이 만들어지고 이들간의 관계를 통해 코드를 통해 봤던 과정들이 실행 됩니다.
이는 실행컨텍스트에 비해 훨씬 비효율적입니다.
따라서 실행 컨텍스트는 식별자 결정을 더욱 효율적으로 하기 위한 수단이라고 말할 수 있겠습니다.

## 5. 최종 정리
2장에서는 자바스크립트엔진의 생성 / 실행 단계를 알아보고 Record가 어떻게 활용되는지, 또 변수, 함수 호이스팅을 알아봤습니다.
3장에서는 스코프 체인이 무엇이고 어떻게 생성되는지와 outer의 활용을 알아봤습니다.
4장에서는 실행 컨텍스트를 정리했습니다.

- [도서]코어 자바스크립트 핵심 개념과 동작 원리로 이해하는 자바스크립트 프로그래밍
- [도서]모던 자바스크립트 Deep Dive 자바스크립트의 기본 개념과 동작 원리
- [강의]자바스크립트 중고급 자바스크립트의 근본 핵심 이해 
- [테코톡]엘라의 Scope & Clossure
- [테코톡]피터의 Event Loop
- [테코톡]브콜의 This
