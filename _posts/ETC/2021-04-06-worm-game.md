...
|setWormHeadDirection()|this.worms|direction|this.worms 안에 있는 객체들이 자신이 가지고 있는 방향으로 한칸씩 움직인다. 이동 후에는 맨 뒤에 있는 Worm 인스턴스 부터 자신의 앞에 있는 Worm 인스턴스의 direction을 이어 받는다.|
|setWormsDirection()|this.worms||worms 들이 가지고 있는 direction값으로 한칸 씩 이동 시킨다. 이동 후에는 자신의 앞에 있는 worm 인스턴스의 direction을 가지게된다.|
|setEdgeObstacles|this.children||필드의 가장자리를 전부 장애물로 채운다.|

### 엔진 생성자
- function WormEngine( size )
