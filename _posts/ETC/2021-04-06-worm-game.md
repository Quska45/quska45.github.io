...
|killWorm|boolean |false|지렁이를 죽이는 객체인지에 대한 여부. 지렁이의 머리가 닿으면 지렁이는 죽는다.|
...
|setWormHeadDirection()|this.worms|this.worms|this.worms 안에 있는 객체들이 자신이 가지고 있는 방향으로 한칸씩 움직인다. 이동 후에는 맨 뒤에 있는 Worm 인스턴스 부터 자신의 앞에 있는 Worm 인스턴스의 direction을 이어 받는다.|
|moveWorms()|this.worms||worms 들이 가지고 있는 direction값으로 한칸 씩 이동 시킨다. 이동 후에는 자신의 앞에 있는 worm 인스턴스의 direction을 가지게된다.|
|setEdgeWalls|this.children||필드의 가장자리를 전부 벽으로 채운다.|
...
|변수명|return타입|인자|설명|
|------|---|---|---|secl
|move()|wbObject|id, {x:x, y:y}|객체를 인자의 위치로 이동시킨다. 해당위치에 이미 객체가 있다면 이동 불가.|
