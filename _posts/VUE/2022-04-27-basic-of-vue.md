...
## Vue Router
 * 회사 시스템이 뒤로 가기를 눌렀을 때 제대로 동작하지 않는 걸 보면서 짜증이 났었습니다. 도대체 어떻게 만들어 졌길래 이딴식으로 동작이 되나 싶었져. vue router에 대해서 알고 나니 이해가 조금은 됐습니다. vue로 만들어진 spa 앱에서 vue router를 이용하는 것은 어쩔 수 없는 한계를 가지고 있다는 것을 알게 됐습니다. 웹서버를 통해 개별 html을 받아오는게 아니기 때문에 근본적으로 같은 동작이 불가능 했던거죠. 하지만 또 하나 느끼는 것은 이런 문제가 있었다면 당연히 이걸 개선할 방법을 검토하고 그렇게 됐어야 하는데 그렇지 않은 것은 여전히 짜증이 났습니다. 이번 회사에서 이것을 개선 할 수 있는 기회가 있을 지는 모르겠지만 할 수만 있다면 한 단계 성장할 수 있겠다는 생각도 들었습니다.
 * code spliting : spa가 가지는 근본적인 문제 중에 하나를 해결할 수 있는 방법으로 소개 됐습니다. 모든 프론트 관련된 소스를 한 번에 로드 하기 때문에 부하가 클 수 밖에 없는데 이걸 나눠서 로드 할 수 있도록 하는 개념입니다. lazy load를 가능하게 하는 거져. 물론 지금 회사 소스는 이런게 안되어 있습니다. 열받는게 저희 회사에 시스템 인수인계를 해준 회사에서 하는 소리가 소스의 규모가 크고 안쓰는 소스가 많기 때문에 이걸 제거 하는 과정을 한 번 거치셔야 될거라고 하더군요. SI를 계속 할게 아니라면, 프리 할게 아니라면 빨리 이직하는게 답이지 않나 싶습니다.
